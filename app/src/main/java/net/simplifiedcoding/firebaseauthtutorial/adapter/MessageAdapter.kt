    ...
    init {
        ...
        getSummonerInfoRef(nickname).get().addOnSuccessListener { info ->
            ...
            Firebase.storage.reference.child("Tier/${info.get("tier").toString()}.png").downloadUrl.addOnSuccessListener { uri ->
                Picasso.get().load(uri).into(findViewById<ImageView>(R.id.profileicon))
                ...
                findViewById<TextView>(R.id.topwinrate).text =
                    (info.get("T_Win") as Double).times(100).toString() + "%"
                findViewById<TextView>(R.id.junglewinrate).text =
                    (info.get("J_Win") as Double).times(100).toString() + "%"
                findViewById<TextView>(R.id.middlewinrate).text =
                    (info.get("M_Win") as Double).times(100).toString() + "%"
                findViewById<TextView>(R.id.bottomwinrate).text =
                    (info.get("B_Win") as Double).times(100).toString() + "%"
                findViewById<TextView>(R.id.supportwinrate).text =
                    (info.get("S_Win") as Double).times(100).toString() + "%"
                findViewById<TextView>(R.id.totalWinRate).text =
                    (info.get("Total_Win") as Double).times(100).toString() + "%"
