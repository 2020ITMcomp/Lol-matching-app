...
class MatchHistory : Fragment() {
    ...
    private fun historyPopulate() {
        ...
        getSummonerGamedataRef(nickname).orderBy("gameId", Query.Direction.DESCENDING).limit(20)
            .addSnapshotListener { totalGamedata, firebaseFirestoreException ->
//            totalGamedata!!.documents
                for (gamedata in totalGamedata!!) {

                    Log.d(TAG, gamedata.id)
                    // 참가자 번호 얻는 로직
                    val participantsIdentity =
                        gamedata.get("participantIdentities") as ArrayList<Map<*, *>>
